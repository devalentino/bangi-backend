...
```bash
docker build -t ghcr.io/devalentino/bangi-backend:$(git describe --tags --exact-match) docker build -t ghcr.io/devalentino/bangi-backend:$(git describe --tags --exact-match) .
```
