...
def migrate(migrator: Migrator, database: pw.Database, *, fake=False):
    ...
    @migrator.create_model
    class BusinessManager(pw.Model):
        id = pw.AutoField()
        created_at = pw.TimestampField(null=True)
        name = pw.CharField(max_length=255)

        class Meta:
            table_name = "facebook_autoregs_business_manager"
...
def rollback(migrator: Migrator, database: pw.Database, *, fake=False):
    ...
    migrator.change_fields('campaign', cost_value=pw.DecimalField(auto_round=False, decimal_places=5, default=Decimal('0'), max_digits=10, null=True, rounding=ROUND_DOWN))

    migrator.remove_model('facebook_autoregs_executor')

    migrator.remove_model('facebook_autoregs_business_manager')
