...
def test_get_business_pages(client, authorization, business_page):
    response = client.get('/api/v2/facebook/pacs/business-pages', headers={'Authorization': authorization})
...
def test_create_business_page(client, authorization, business_page_name, read_from_db):
    ...
    response = client.post(
        '/api/v2/facebook/pacs/business-pages', headers={'Authorization': authorization}, json=request_payload
    )
    ...
    db_payload = read_from_db('facebook_pacs_business_page')
    assert db_payload == {
        'id': mock.ANY,
        'created_at': mock.ANY,
        'name': request_payload['name'],
...
def test_get_business_page(client, authorization, business_page):
    response = client.get(
        f'/api/v2/facebook/pacs/business-pages/{business_page["id"]}',
...
def test_get_business_page__non_existent(client, authorization):
    response = client.get('/api/v2/facebook/pacs/business-pages/100500', headers={'Authorization': authorization})
...
def test_update_business_page(client, authorization, business_page, read_from_db):
    ...
    db_payload = read_from_db('facebook_pacs_business_page', filters={'id': business_page['id']})
    assert db_payload == {
        'id': mock.ANY,
        'created_at': mock.ANY,
        'name': request_payload['name'],
